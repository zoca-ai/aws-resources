          ...
          <DialogHeader>
            ...
            <DialogDescription>
              {isNullMapping ? (
                selectedOldResources.length === 0 ? (
                  <>
                    You are about to mark {selectedNewResources.length} resource
                    {selectedNewResources.length > 1 ? "s" : ""} as newly added
                    to the infrastructure.
                  </>
                ) : (
                  <>
                    You are about to mark {selectedOldResources.length} old
                    resource
                ...
                <div className="text-sm">
                  {isMultipleOldMapping && isMultipleNewMapping && (
                    <div className="mb-1 text-yellow-800">
                      <strong>Many-to-Many Mapping:</strong> This will create a
                      single many-to-many mapping linking{" "}
                      {selectedOldResources.length} old resources to{" "}
                      {selectedNewResources.length} new resources.
                    </div>
                  )}
                  {isMultipleOldMapping && !isMultipleNewMapping && (
                    <div className="mb-1 text-yellow-800">
                      <strong>Many-to-One Mapping:</strong> This will create a
                      single mapping from {selectedOldResources.length} old
                      resources to 1 new resource.
                    </div>
                  )}
                  {!isMultipleOldMapping && isMultipleNewMapping && (
                    <div className="mb-1 text-yellow-800">
                      <strong>One-to-Many Mapping:</strong> This will create a
                      single mapping from 1 old resource to{" "}
                      {selectedNewResources.length} new resources.
                    </div>
                  )}
                  {isNullMapping && selectedOldResources.length === 0 && (
                    <div className="mb-1 text-yellow-800">
                      <strong>Newly Added Resources:</strong> These resources
                      will be marked as newly added to the infrastructure with
                      no legacy equivalent.
                    </div>
                  )}
                  {isNullMapping && selectedNewResources.length === 0 && (
                    <div className="mb-1 text-yellow-800">
                      <strong>Migration Marking:</strong> These legacy resources
                      will be marked for migration, deprecation, or removal.
                    </div>
                  )}
                  {isCrossTypeMapping && (
