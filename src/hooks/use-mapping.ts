...
export interface SeparateFiltersState {
  old: FilterState;
  new: FilterState;
}
...
export interface UseMapping {
  ...
  handleUpdateMappingNotes: (mappingId: number, notes: string) => Promise<void>;
  ...
  handleMapFromNothing: (resourceIds: string[], notes?: string) => Promise<any>;
...
export function useMapping(): UseMapping {
  // Filter states - separate for old and new resources
  const [filters, setFilters] = useState<SeparateFiltersState>({
    old: {
      search: "",
      type: "all",
      region: "all",
  ...
  // Debounced search - separate for old and new
  ...
  const oldQueryInput = useMemo(
    () => ({
      ...
      ...(filters.old.mappingStatus !== "all" && {
        mappingStatus: filters.old.mappingStatus as "unmapped" | "mapped",
      }),
    }),
    [
      debouncedOldSearch,
  ...
  const newQueryInput = useMemo(
    () => ({
      ...
      ...(filters.new.mappingStatus !== "all" && {
        mappingStatus: filters.new.mappingStatus as "unmapped" | "mapped",
      }),
    }),
    [
      debouncedNewSearch,
  ...
  } = api.resources.categorizedInfinite.useInfiniteQuery(oldQueryInput, {
  ...
  } = api.resources.categorizedInfinite.useInfiniteQuery(newQueryInput, {
    getNextPageParam: (lastPage) => lastPage.nextCursor,
    staleTime: 2 * 60 * 1000,
  });
  ...
  } = api.migration.mappingsInfinite.useInfiniteQuery(mappingsQueryInput, {
    getNextPageParam: (lastPage) => lastPage.nextCursor,
    staleTime: 1 * 60 * 1000,
  ...
  const createBulkManyToManyMapping =
    api.migration.createBulkManyToManyMapping.useMutation({
      onMutate: async (variables) => {
        toast.loading("Creating bulk many-to-many mappings...", {
      ...
      onSuccess: (data) => {
        toast.success(
          `Successfully created many-to-many mapping: ${data.sourceCount} sources → ${data.targetCount} targets`,
          { id: "create-bulk-mapping" },
        );
  ...
  const createNewResourceMapping =
    api.migration.createNewResourceMapping.useMutation({
      onMutate: async (variables) => {
        toast.loading("Marking resources as newly added...", {
      ...
      onSuccess: () => {
        toast.success("Resources marked as newly added", {
          id: "create-new-mapping",
        });
        utils.migration.mappingsInfinite.invalidate();
      },
  ...
  const transformResourcesWithMappingStatus = useCallback(
    (resourceList: Resource[]): MappingResource[] => {
      ...
      return resourceList.map((resource): MappingResource => {
        // Find mapping for this resource (as source or target)
        const relatedMapping = mappings.find(
          (m) =>
            m.sourceResources?.some(
              (s) => s.resourceId === resource.resourceId,
            ) ||
            (m as any).targetResources?.some(
              (t: any) => t.resourceId === resource.resourceId,
            ),
        );

        // Determine status: pending overrides server filtering, otherwise assume server got it right
        const mappingStatus: MappingStatus = pendingResourceIds.has(
          resource.resourceId,
        )
          ? "pending"
          : relatedMapping
            ? "mapped"
            : "unmapped"; // Fallback logic since we don't know category here

        return {
          ...resource,
          mappingStatus,
          mappedToResourceId: relatedMapping?.id?.toString(),
          mappedToResourceName:
            relatedMapping?.sourceResources?.[0]?.resourceName || undefined,
  ...
  useEffect(() => {
    const prefetchTimer = setTimeout(() => {
      ...
      if (
        oldResources.length > 25 &&
        hasNextOldPage &&
        !isFetchingNextOldPage
      ) {
        fetchNextOldPage();
      }
      // Prefetch next new resources page
      if (
        newResources.length > 25 &&
        hasNextNewPage &&
        !isFetchingNextNewPage
      ) {
        fetchNextNewPage();
      }
  ...
  const updateFilter = useCallback(
    (category: "old" | "new", field: keyof FilterState, value: string) => {
      setFilters((prev) => ({
        ...prev,
        [category]: {
          ...prev[category],
    ...
    async (resourceId: string, mappingType = "deprecation", notes?: string) => {
      try {
        await createMapping.mutateAsync({
          ...
          mappingDirection: "old_to_new",
          mappingType:
            mappingType === "removal" ? "to_be_removed" : (mappingType as any),
    ...
    async (
      ...
      try {
        const result = await createBulkManyToManyMapping.mutateAsync({
          ...
          notes:
            notes ||
            `${resourceIds.length} resources marked for ${mappingType}`,
          priority: "medium",
        });
    ...
    async (resourceIds: string[], notes?: string) => {
      try {
        const result = await createBulkManyToManyMapping.mutateAsync({
          ...
          notes:
            notes || `${resourceIds.length} resources marked as newly added`,
          priority: "medium",
        });
